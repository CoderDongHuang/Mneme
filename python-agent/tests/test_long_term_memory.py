...
class TestPreferences:
    ...
    def test_duplicate_not_added(self, ltm):
        ...
        with patch.object(
            ltm._store,
            "find_duplicates",
            return_value=[{"id": "mem_old", "content": "喜欢图表", "score": 0.05}],
        ):
...
class TestWeakPoints:
    ...
    def test_same_topic_merges(self, ltm):
        ...
        with (
            patch.object(ltm._store, "get_by_category", return_value=existing),
...
class TestGetAllMemories:
    def test_returns_all_categories(self, ltm):
        """获取所有记忆返回三类数据"""
        with (
            patch.object(ltm, "get_preferences", return_value=["pref1"]),
