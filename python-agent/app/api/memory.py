...
@router.post(
    "/read",
    summary="读取用户记忆",
    description="读取指定用户的偏好、薄弱点和学习进度",
...
@router.post(
    "/write",
    summary="写入记忆",
    description="手动写入一条长期记忆（偏好/薄弱点/进度）",
...
async def write_memory(request: MemoryWriteRequest):
    ...
    if entry.category == "preference":
        ...
        long_term_memory.add_weak_point(
            request.user_id, entry.content, entry.topic or entry.content
        )
...
@router.post(
    "/confirm",
    summary="确认/拒绝记忆",
    description="用户对蒸馏产出的中等置信度记忆进行确认或拒绝",
)
